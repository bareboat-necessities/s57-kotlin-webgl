...
## Second chunk implementation status

Implemented in the second chunk:

- Added the first common `RenderPipelineDiagnostic` and
  `RenderPipelineDiagnosticReport` model with stage, severity, stable code,
  feature/object context, plain-text export, and JSON export.
- Extended `S52RenderSummary` and `RenderedFrameSummary` so browser S-52 adapter
  diagnostics and fallback diagnostics can be carried with render results.
- Converted browser S-52 adapter warnings for unsupported objects, unsupported
  attributes, unsupported primitives, invalid polygon rings, split multipolygons,
  and SOUNDG multipoint mismatches into structured diagnostics.
- Added browser console warning/error logging for render pipeline diagnostics and
  S-52 geometry fallback events.
- Updated the demo render status text to include the structured render pipeline
  diagnostic report whenever diagnostics are present.
- Added common tests for diagnostic JSON/plain-text export, aggregate counters,
  and rendered-frame diagnostic collection.

Remaining Phase 26 todo:

1. Thread the same structured diagnostic model through raw ISO8211/S-57 import,
   raw decode, geometry reconstruction, indexing, projection, and the existing
   JVM visual-smoke path.
2. Add S-52 asset-level diagnostics for missing symbol names, line styles, area
   patterns, text instructions, color tokens, resolved RGB values, and fallback
   colors emitted by the companion S-52 renderer.
3. Add explicit JSON download and PNG snapshot buttons to the demo UI instead of
   only showing diagnostics in the status panel.
4. Build the headless browser/Playwright snapshot harness that writes
   `build/ci-enc-snapshot/render.png`, `diagnostics.json`, and `summary.txt`.
5. Add `.github/scripts/download-first-enc-cell.sh` and wire the GitHub Actions
   artifact upload step for the downloaded public ENC cell snapshot.
6. Expand the S-57 catalogue and attribute normalization so adapter conversion is
   catalogue-driven rather than ad hoc name sets.
7. Add a public-cell regression corpus with per-cell coverage thresholds for
   unknown objects, missing symbols, missing colors, fallback draws, and skipped
   features.
8. Tighten CI from warning-only fallback counts to failing quality gates once the
   corpus and S-52 asset coverage are stable.

## Third chunk implementation status

Implemented in the third chunk:

- Added an exportable `RenderSnapshotDiagnosticExport` JSON bundle for demo and
  future CI artifacts.
- Added JSON serializers for rendered artifact counters and S-52 render summary
  counters.
- Added demo buttons to download the latest render diagnostics JSON and a PNG
  snapshot of the current canvas.
- Reset stale downloadable diagnostics when imports, cache restores, sample
  renders, or clear operations replace the active chart state.
- Added tests for the combined snapshot diagnostics JSON export.

Remaining Phase 26 todo after this chunk:

1. Thread structured diagnostics through ISO8211/S-57 import, raw decode,
   geometry reconstruction, indexing, projection, and JVM visual smoke.
2. Add S-52 asset-level diagnostics from the companion renderer for missing
   symbols, line styles, patterns, text instructions, color tokens, resolved RGB
   values, and fallback colors.
3. Build the headless browser/Playwright snapshot harness that writes
   `build/ci-enc-snapshot/render.png`, `diagnostics.json`, and `summary.txt`
   using the same exported JSON shape as the demo button.
4. Add `.github/scripts/download-first-enc-cell.sh` and wire the GitHub Actions
   snapshot artifact upload.
5. Expand the S-57 catalogue and replace remaining ad hoc attribute conversion
   sets with catalogue-driven normalization.
6. Add a public-cell regression corpus with per-cell coverage thresholds.
7. Tighten CI from warning-only fallback counts to failing quality gates once the
   corpus and S-52 asset coverage are stable.
